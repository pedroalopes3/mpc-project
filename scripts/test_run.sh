...
docker compose exec -T party0 bash -lc \
  "cd /mp-spdz && ./shamir-party.x -N $nparties -p 0 -ip Config/IPs -IF Inputs/Input $prog" \
  2>&1 | tee "$logdir/party0.log" &
p0=$!

docker compose exec -T party1 bash -lc \
  "cd /mp-spdz && ./shamir-party.x -N $nparties -p 1 -ip Config/IPs -IF Inputs/Input $prog" \
  > "$logdir/party1.log" 2>&1 &
p1=$!

docker compose exec -T party2 bash -lc \
  "cd /mp-spdz && ./shamir-party.x -N $nparties -p 2 -ip Config/IPs -IF Inputs/Input $prog" \
